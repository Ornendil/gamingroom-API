...
if ( isset($_POST['id']) && isset($_POST['lnr']) ) {
    $id = $_POST['id'];
    $lnr = $_POST['lnr'];

    // Prepare an SQL statement to update the lnr
    $stmt = $db->prepare('UPDATE gaming_sessions SET lnr = :lnr WHERE id = :id');
    ...
    if ($stmt->execute()) {
        echo json_encode(['status' => 'success', 'id' => $id, 'lnr' => $lnr]);
    } else {
        writeLog("[Error] Failed to update lnr for session.");
        echo json_encode(['status' => 'error', 'message' => 'Failed to update session.', 'id' => $id, 'lnr' => $lnr]);
    }
} else {
    writeLog("[Error] Couldn't update lnr for session. Missing required parameters.");
